...
def train_distill(args):
    ...
    val_ds = LOLDataset(mode="val", layout=args.layout, root=args.dataset_root, augment=False)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (low, high) in enumerate(pbar):
            ...
            # --- Sample a discrete timestep from the student's sparse schedule ---
            b = low.shape[0]
            idx = torch.randint(0, len(student_schedule), (b,), device=conf.DEVICE)
            t_student = student_schedule[idx]
            ...
            if args.w_distill > 0.0:
                with torch.no_grad():
                    ...
                    if getattr(conf, "OFFSET_NOISE_STRENGTH", 0.0) > 0:
                        noise = diff.make_noise(low, conf.OFFSET_NOISE_STRENGTH)

                    # We want to diffuse the true residual to t_student
                    true_residual = torch.clamp(high - low, -1.0, 1.0)
                    x_t_student, _ = diff.q_sample(true_residual, t_student, noise=noise)

                    # Teacher takes multiple steps down to 0 to provide a refined target
                    # (Uses its own schedule starting from t_student down to 0)
                    refined_residual = teacher_refine(
                        teacher, diff, x_t_student, low,
                        t_start=t_student[0].item(), num_substeps=args.teacher_steps
                    )
            else:
                # Anchor-only baseline: no teacher distillation. Diffuse true residual directly.
                noise = torch.randn_like(low)
                if getattr(conf, "OFFSET_NOISE_STRENGTH", 0.0) > 0:
                    noise = diff.make_noise(low, conf.OFFSET_NOISE_STRENGTH)
                true_residual = torch.clamp(high - low, -1.0, 1.0)
                x_t_student, _ = diff.q_sample(true_residual, t_student, noise=noise)
                refined_residual = None  # No teacher distillation

            # Student makes a 1-step prediction of the residual
            _, student_residual = student(x_t_student, t_student, low, return_residual=True)
            student_residual = torch.clamp(student_residual, -1.0, 1.0)

            loss_distill = torch.tensor(0.0, device=conf.DEVICE)
            if args.w_distill > 0.0:
                loss_distill = charbonnier(student_residual, refined_residual)
            loss_anchor = charbonnier(student_residual, true_residual)

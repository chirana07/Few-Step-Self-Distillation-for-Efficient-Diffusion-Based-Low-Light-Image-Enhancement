...
def fixed_latent_like(tensor, base_seed, sample_id):
    digest = hashlib.sha256(f"{base_seed}:{sample_id}".encode("utf-8")).digest()
    latent_seed = int.from_bytes(digest[:8], "big") % (2 ** 63 - 1)
...
@torch.no_grad()
def validate(model, diff, val_loader, device, inference_steps, seed=3407, max_batches=None):
    ...
    for i, batch in enumerate(val_loader):
        if max_batches is not None and i >= max_batches:
            break
        low, high, sample_ids = batch
        low = low.to(device)
        high = high.to(device)
        initial_noise = fixed_latent_like(low, seed, sample_ids[0])
        pred = diff.ddim_sample(
            model,
            low,
...
def train(args):
    ...
    val_ds = LOLDataset(mode="val", layout=args.layout,
                        root=args.dataset_root, augment=False,
                        manifest_path=args.manifest, manifest_split="val",
                        return_name=True)
    ...
    log_path = os.path.join(save_dir, f"train_log_{args.tag or 'default'}.csv")
    ...
    for epoch in range(conf.EPOCHS):
        ...
        if (epoch + 1) % conf.VAL_EVERY == 0 or epoch == conf.EPOCHS - 1:
            ...
            if val_psnr > best_psnr:
                ...
                torch.save({
                    ...
                    "validation_inference_steps": args.inference_steps,
                    "validation_seed": args.val_seed,
                }, os.path.join(save_dir, f"best_{args.tag or 'default'}.pth"))
        ...
        torch.save({
            ...
            "use_illum_prior": conf.USE_ILLUM_PRIOR,
            "manifest": os.path.abspath(args.manifest),
        }, os.path.join(save_dir, f"last_{args.tag or 'default'}.pth"))
...
def parse():
    ...
    p.add_argument("--manifest", required=True,
                   help="JSON train/validation manifest created from the training set only.")
    p.add_argument("--output-dir", default=None)
    p.add_argument("--inference-steps", type=int, default=5,
                   help="Explicit validation DDIM step count.")

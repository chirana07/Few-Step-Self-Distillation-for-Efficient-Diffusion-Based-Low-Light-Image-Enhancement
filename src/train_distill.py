...
def train_distill(args):
    ...
    if args.use_illum_prior:
        raise ValueError("The verified transferred teacher uses the six-channel architecture")
    use_prior = False

    print(f"=== Few-step self-distillation ===")
    print(f"  run_kind        = {args.run_kind}")
    print(f"  init_from       = {args.init_from}")
    print(f"  teacher_from    = {args.teacher_from or args.init_from}")
    ...
    if not args.manifest:
        raise SystemExit("--manifest is required; generate it with scripts/make_lolv2_split.py")

    train_ds = LOLDataset(
        mode="train", layout=args.layout, root=args.dataset_root,
        crop_size=conf.CROP_SIZE, augment=True,
        manifest_path=args.manifest, manifest_split="train",
    )
    val_ds = LOLDataset(
        mode="val", layout=args.layout, root=args.dataset_root, augment=False,
        manifest_path=args.manifest, manifest_split="val", return_name=True,
    )

    loader_generator = torch.Generator()
    loader_generator.manual_seed(args.seed or conf.SEED)
    ...
    teacher = None
    teacher_path = args.teacher_from or args.init_from
    if args.run_kind == "fsd":
        raw_ckpt = torch.load(teacher_path, map_location="cpu")
        teacher_state = raw_ckpt.get("ema", raw_ckpt.get("model", raw_ckpt)) if isinstance(raw_ckpt, dict) else raw_ckpt
        head_channels = int(teacher_state["head.weight"].shape[1])
        if head_channels != 6:
            raise ValueError(f"Expected six-channel teacher head, found {head_channels}")
        if isinstance(raw_ckpt, dict) and raw_ckpt.get("use_illum_prior", False):
            print("[teacher] ignoring stale use_illum_prior=True metadata; head has 6 channels")
        teacher_use_prior = False
        del raw_ckpt
        teacher = ResidualConditionedUNet(use_illum_prior=teacher_use_prior).to(conf.DEVICE)
        smart_load_checkpoint(teacher, teacher_path, conf.DEVICE,
                              prefer="ema" if not args.init_from_model_key else "model")
        teacher.eval()
        ...
        if args.w_distill != 0.0:
            raise ValueError("anchor_only requires --w-distill 0")
        print("[teacher] skipped for anchor-only continued-training control")
    ...
    print(f"[student] coarse {n_student_steps}-step schedule (timesteps): {student_schedule.tolist()}")

    # --- logging ---
    save_dir = args.output_dir or conf.SAVE_DIR
    os.makedirs(save_dir, exist_ok=True)
    log_path = os.path.join(save_dir, f"distill_log_{args.tag}.csv")
    ...
    for epoch in range(args.epochs):
        ...
        for low, high in pbar:
            ...
            # Losses
            if teacher is None:
                loss_distill = student_x0.new_zeros(())
            else:
        ...
        if (epoch + 1) % args.val_every == 0 or epoch == args.epochs - 1:
            ...
            val_psnr, val_ssim = validate(
                ema_model, diff, val_loader, conf.DEVICE,
                inference_steps=n_student_steps,
                seed=args.val_seed,
                max_batches=args.val_max_batches,
            )
            print(f"[epoch {epoch}] {n_student_steps}-step val PSNR {val_psnr:.3f}  SSIM {val_ssim:.4f}")
            ...
            if val_psnr > best_psnr:
                ...
                torch.save({
                    ...
                    "teacher_from": teacher_path if teacher is not None else None,
                    "w_distill": args.w_distill,
                    "w_anchor": args.w_anchor,
                }, os.path.join(save_dir, f"best_{args.tag}.pth"))
        ...
        torch.save({
            ...
            "w_distill": args.w_distill,
            "w_anchor": args.w_anchor,
        }, os.path.join(save_dir, f"last_{args.tag}.pth"))
    ...
    log_f.close()
    print(f"Done. Best {n_student_steps}-step val PSNR: {best_psnr:.3f}")
...
def parse():
    ...
    p.add_argument("--manifest", required=True,
                   help="JSON train/validation manifest generated from the training set only.")
    p.add_argument("--output-dir", default=None)
    p.add_argument("--run-kind", choices=["fsd", "anchor_only"], default="fsd")
    p.add_argument("--init-from", required=True,
                   help="Path to the Day 1 checkpoint. Used for BOTH teacher and student init.")
    p.add_argument("--teacher-from", default=None,
                   help="Optional frozen teacher checkpoint; defaults to --init-from.")
    ...
    p.add_argument("--val-seed", type=int, default=3407,
                   help="Fixed per-image latent seed namespace used for validation.")

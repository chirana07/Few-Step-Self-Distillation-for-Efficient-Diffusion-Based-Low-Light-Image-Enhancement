...
class LOLDataset(Dataset):
    ...
    def __init__(
        ...
        if manifest_path is not None:
            ...
            key = manifest_split or mode
            if key not in manifest.get("splits", {}):
                raise KeyError(f"Manifest {manifest_path} has no split '{key}'")
            requested = [
                (item["low"], item["high"])
                for item in manifest["splits"][key]
            ]
            available = set(self.pairs)
            missing = [pair for pair in requested if pair not in available]
            if missing:
                raise RuntimeError(
                    f"Manifest contains {len(missing)} pairs not found under "
                    f"{self.low_dir} <-> {self.high_dir}; first missing pair: {missing[0]}"
                )
            self.pairs = requested
